...
namespace Offsets {
    ...
    kaddr LineOfSightTo_Func = 0x104cfc5f4;
}
...
string GetFName(kaddr actorAddress) {
    ...
    kaddr gname_1 = GetRealOffset(0x103d384c8);
    kaddr gname_2 = GetRealOffset(0x108373600);
...
string getWeaponWithFName(string FName) {
   ...
   if (isContain(FName, "BP_Sniper_SKS_Wrapper_C")) {
    ...
    if (isContain(FName, "BP_MachineGun_Thompson_Wrapper_C")) {
        return "Tomygun";
    }
...
- (void)readData
{
    ...
    kaddr gworld_1 = GetRealOffset(0x10276dd30);
    kaddr gworld_2 = GetRealOffset(0x108701448);
...
#pragma mark
            ...
            auto rootComponent = GetPtr(props.base + 0x1C0);
...
- (bool)readLocalPlayerInfo
{
    ...
    self.localPlayer.base = GetPtr(localPlayerController + 0x3E0);
    ...
    if (IsValidAddress(self.localPlayer.base)) {
        ...
        if (isPlayer(FName)) {
            if ([self readPlayerInfo:self.localPlayer]) {
                self.isFire = Read<bool>(self.localPlayer.base + 0x1500);
            auto weaponManagerComponent = GetPtr(self.localPlayer.base + 0x1EC8);
                auto cachedCurUseWeapon = GetPtr(weaponManagerComponent + 0x450);
    ...
    BulletTrack = playerCameraManager + 0xB4C;
...
{
    auto rootComponent = GetPtr(player.base + 0x1C0);
    if (IsValidAddress(rootComponent)) {
    ...
    player.hp = Read<float>(player.base + 0xCE0);
    player.maxHp = Read<float>(player.base + 0xCE4);
    player.signalHP = Read<float>(player.base + 0xce0);
    player.signalHPMax = Read<float>(player.base + 0xce4);
    player.isFallDown = player.hp == 0;
    
    player.isDead = Read<bool>(player.base + 0xCFC);
    ...
    auto nameAddr = GetPtr(player.base + 0x890);
    ...
    player.playerKey = Read<UInt32>(player.base + 0x8B0);
    player.isAI = Read<bool>(player.base + 0x968);
    player.teamNo = Read<int>(player.base + 0x8D8);
...
- (void)modifyWeaponData
{
    if (IsValidAddress(ownerShootWeapon)) {
        kaddr shootWeaponEntityComp = GetPtr(ownerShootWeapon + 0xF48);
        
        
        
        if (self.overlayView.isNorecoil && self.isFire) {
            if (IsValidAddress(shootWeaponEntityComp)) {
                Write<float>(shootWeaponEntityComp + 0xA20, 0.001);//无后
                Write<float>(shootWeaponEntityComp + 0xA24, 0.001);
...
- (void)aimbot
{
    ...
    if (self.isFire) {
        if (IsValidAddress(self.lockActor)) {
            ...
            float localPlayerHp = Read<float>(self.localPlayer.base + 0xCCC);
